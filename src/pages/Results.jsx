...
import {
  ...
  ArrowLeft, Download, TrendingUp, Info
} from 'lucide-react';
...
function Results() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from('.results-topbar',   { opacity: 0, y: -30, duration: 0.6, ease: 'power3.out' });
      gsap.from('.results-summary',  { opacity: 0, y:  40, duration: 0.7, delay: 0.1, ease: 'power3.out' });
      gsap.from('.stat-card',        { opacity: 0, y:  50, duration: 0.6, stagger: 0.1, delay: 0.2, ease: 'power3.out' });
      gsap.from('.results-tabs',     { opacity: 0, y:  20, duration: 0.5, delay: 0.5, ease: 'power2.out' });
    }, containerRef);
    return () => ctx.revert();
  }, [results]);
  ...
  return (
    <div className="results-container" ref={containerRef}>
      ...
      {/* ── TOP BAR ── */}
      <div className="results-topbar container">
      ...
      <div className="stats-grid container">
        ...
        <div className="stat-card panel">
          <div className="stat-icon-wrapper" style={{ background: RISK_BG.High, color: RISK_COLORS.High }}>
            <AlertTriangle size={28} />
          </div>
          <div className="stat-info">
            <h3>High Risk</h3>
            <p className="stat-number" style={{ color: RISK_COLORS.High }}>{highRisk.toLocaleString()}</p>
          </div>
        </div>
        <div className="stat-card panel">
          <div className="stat-icon-wrapper" style={{ background: RISK_BG.Medium, color: RISK_COLORS.Medium }}>
            <Activity size={28} />
          </div>
          <div className="stat-info">
            <h3>Medium Risk</h3>
            <p className="stat-number" style={{ color: RISK_COLORS.Medium }}>{mediumRisk.toLocaleString()}</p>
          </div>
        </div>
        <div className="stat-card panel">
          <div className="stat-icon-wrapper" style={{ background: RISK_BG.Low, color: RISK_COLORS.Low }}>
            <ShieldCheck size={28} />
          </div>
          <div className="stat-info">
            <h3>Low Risk / Safe</h3>
            <p className="stat-number" style={{ color: RISK_COLORS.Low }}>{lowRisk.toLocaleString()}</p>
          </div>
        </div>
        <div className="stat-card panel">
      ...
      {activeTab === 'overview' && (
        <div className="tab-content container">
          ...
          <div className="plain-summary panel">
          ...
          <div className="overview-charts">
            ...
            <div className="chart-card panel">
              ...
              <div className="donut-wrap">
                <svg viewBox="0 0 42 42" className="donut-svg">
                  ...
                  {hPct > 0 && (
                    <circle cx="21" cy="21" r={r} fill="transparent"
                      stroke={RISK_COLORS.High} strokeWidth="4"
                  ...
                  {mPct > 0 && (
                    <circle cx="21" cy="21" r={r} fill="transparent"
                      stroke={RISK_COLORS.Medium} strokeWidth="4"
                  ...
                  {lPct > 0 && (
                    <circle cx="21" cy="21" r={r} fill="transparent"
                      stroke={RISK_COLORS.Low} strokeWidth="4"
            ...
            {featureImportance.length > 0 && (
              <div className="chart-card panel">
                <h3 className="chart-title">Top Churn Drivers</h3>
      ...
      {activeTab === 'customers' && (
        <div className="tab-content container">
              ...
              {['All', 'High', 'Medium', 'Low'].map(r => (
                <button
                  ...
                  style={filterRisk === r && r !== 'All'
                    ? { background: RISK_BG[r], color: RISK_COLORS[r], borderColor: RISK_COLORS[r] }
                    : {}}
          ...
          <div className="customer-table-wrap panel">
            <table className="customer-table">
              ...
              <tbody>
                {paginated.length === 0 ? (
                  <tr><td colSpan="4" style={{ textAlign:'center', padding:'30px', color:'var(--text-muted)' }}>No customers found.</td></tr>
                ) : paginated.map((c, i) => (
                  <tr key={i}>
                    <td className="customer-id">{c.id}</td>
                    <td>
                      <div className="prob-cell">
                        <span style={{ color: RISK_COLORS[c.risk], fontWeight: 600 }}>{c.probability}%</span>
                    ...
                    <td>
                      <span className="risk-badge"
                        style={{ background: RISK_BG[c.risk], color: RISK_COLORS[c.risk], borderColor: RISK_COLORS[c.risk] }}>
                        {c.risk} Risk
                      </span>
                    </td>
        ...
        <div className="tab-content container">
          <div className="insights-grid">
            ...
            {[
              ...
              <div key={card.risk} className="insight-card panel"
                style={{ borderLeft: `4px solid ${RISK_COLORS[card.risk]}` }}>
            ...
            <div className="insight-card panel" style={{ borderLeft: '4px solid #6366f1' }}>
              <div className="insight-card-header">
                <span className="insight-risk-badge"
